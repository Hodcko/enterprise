<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<html>
<body style="background-color: powderblue">
<h2 style="text-align: center"><c:out value="Регистрация преподавателя"/></h2>

<form style="text-align: center" action="${pageContext.request.contextPath}/validation" method="post">
    <fieldset>
        <legend><c:out value="Регистрационные данные"/></legend>
        <br><label for="name">Имя</label>
        <br><input id="name" type="text" name="name"><br/>
        <br><label for="secondName">Фамилия</label>
        <br><input id="secondName" type="text" name="secondName"><br/>
        <br><label for="email">Адрес электронной почты</label>
        <br><input id="email" type="text" name="email"><br/>

        <h2 style="text-align: center"><c:out value="Выберите язык программирования для преподавания"/></h2>
        <label>
            <input type="radio" name="langType" value="java">
        </label> Java
        <label>
            <input type="radio" name="langType" value="php">
        </label> PHP
        <label>
            <input type="radio" name="langType" value="c++">
        </label> C++
        <br><input type="submit" name="userType" value="teacher">
    </fieldset>

</form>
</body>
</html>